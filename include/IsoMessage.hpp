...
namespace isolib
{
  ...
  public:
    bool get(size_t idx) const {
        ...
        //std::cout << idx << " : "  << (idx - 1) / 64 << "\n";
        //edit by navid
        auto pos = idx % 64 == 0 ? idx : idx % 64;
        return isolib::get(pos, _bitmap[(idx - 1) / 64]);
    }
  ...
  class IsoMessage
  {
    ...
    void read(const std::string& in)
    {
      ...
      for (size_t i = BitmapLength; i <= _bitmap.size(); i++) {
        ...
        PLOGD << "read Successfully ";
    }

    std::vector<size_t> getAllFields() {
      std::vector<size_t> allFields;
      for (size_t i = BitmapLength; i <= _bitmap.size(); i++) {
        if (_bitmap.get(i))
